...
function elevar(num, exponent) {
  ...
  // Tu código:
  const alexponente = Math.pow(num,exponent);  
  return alexponente;
...
function redondearHaciaArriba(num) {
  ...
  // Tu código:
  const redondearriba = Math.ceil(num);  
...
function numeroRandom(min,max) {
  ...
  let min = 0;
  let max = 1; 
  const numerorandom = Math.random(min,max)
}
